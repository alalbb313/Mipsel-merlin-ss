...
wget --no-check-certificate --timeout=8 -qO - https://raw.githubusercontent.com/alalbb313/Mipsel-merlin-ss/master/ss/rules/version > /tmp/version1
if [ "$?" == "0" ]; then
...
if [ "$ss_basic_gfwlist_update" == "1" ];then
	if [ ! -z "$version_gfwlist2" ];then
		if [ "$version_gfwlist1" != "$version_gfwlist2" ];then
			...
			#wget --no-check-certificate --timeout=8 -qO - https://cokebar.github.io/gfwlist2dnsmasq/dnsmasq_gfwlist_ipset.conf | sed 's/=\//=\/./g' > /tmp/gfwlist.conf
			wget --no-check-certificate --timeout=8 -qO - https://ss-merlin.iloli.li/proxy-github-cokebar/gfwlist2dnsmasq/dnsmasq_gfwlist_ipset.conf | sed 's/=\//=\/./g' > /tmp/gfwlist.conf
			#md5sum_gfwlist1=$(md5sum /tmp/gfwlist.conf | sed 's/ /\n/g'| sed -n 1p)
			md5sum_gfwlist1=$(cat /tmp/version1 | sed -n 1p | sed 's/ /\n/g'| sed -n 1p)
...
if [ "$ss_basic_chnroute_update" == "1" ];then
	if [ ! -z "$version_chnroute2" ];then
		if [ "$version_chnroute1" != "$version_chnroute2" ];then
			...
			wget --no-check-certificate --timeout=8 -qO - https://raw.githubusercontent.com/ACL4SSR/ACL4SSR/master/china_ip_list.txt > /tmp/chnroute.txt
			#md5sum_chnroute1=$(md5sum /tmp/chnroute.txt | sed 's/ /\n/g'| sed -n 1p)
			md5sum_chnroute1=$(cat /tmp/version1 | sed -n 2p | sed 's/ /\n/g'| sed -n 1p)
...
if [ "$ss_basic_cdn_update" == "1" ];then		
	if [ ! -z "$version_cdn2" ];then		
		if [ "$version_cdn1" != "$version_cdn2" ];then		
			...
			wget --no-check-certificate --timeout=8 -qO - https://raw.githubusercontent.com/alalbb313/Mipsel-merlin-ss/master/ss/rules/cdn.txt > /tmp/cdn.txt		
			#md5sum_cdn1=$(md5sum /tmp/cdn.txt | sed 's/ /\n/g'| sed -n 1p)
			md5sum_cdn1=$(cat /tmp/version1 | sed -n 4p | sed 's/ /\n/g'| sed -n 1p)		
